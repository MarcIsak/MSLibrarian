...
#' @param predLib character defining the spectral library to use for prediction
...
#' @export spectral.match.tmp

spectral.match.tmp <- function(msLib, predLib,tolType,tol,trim,mzRange,precursorRm,randIntensity, listName, threads, outFile) {
  ...
  for (i in 1:length(slotStr)) {
    ...
    matches = foreach(j = 1:nrow(slot(msLib@SpectrastLib,slotStr[i])@PrecursorData$FilterLib), .packages = c("tcltk","MSnbase"), .combine = 'rbind') %dopar% {
      ...
      if(!is.na(slot(msLib@SpectrastLib,slotStr[i])@PrecursorData$FilterLib[j,predIdx])) {
        ...
        #                   trimRange)
        predSpec = trimMz(slot(msLib@PredLib, selPred)$Spectra[[j]],
                          trimRange)
        if(precursorRm == T){
